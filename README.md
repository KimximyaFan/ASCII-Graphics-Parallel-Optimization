...
단일보다 멀티쓰레드가 성능이 더 좋게 나오므로

해당 병렬화는 나름 성공적이라 볼 수 있다

제일 성능이 좋게나온 4쓰레드 같은경우, 단일에 비해 1.6배 빠르다

하지만 8쓰레드에서 오히려 성능이 떨어진다

<br>

실행 환경은 i7-13620H CPU를 썼는데, 해당 프로세서는 10코어/16스레드 구조이므로

이론상 8쓰레드도 성능이 괜찮게 나와야한다

쓰레드 수가 많아질수록 락경쟁, 컨텍스트 스위치 오버헤드등이 커지겠지만

각종 수정을 통해 8 쓰레드도 성능이 좋게 나오는 걸 목표로 잡아보도록 한다
